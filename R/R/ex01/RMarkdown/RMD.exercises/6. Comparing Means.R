...
round( rnorm(n = 10, mean = 20, sd = 2), 1 )

df <- data.frame(
  name = paste0(rep('M_', 10), 1:10),
  weight = round(rnorm(n = 10, mean = 20, sd = 20), 1)
)
df
...
#--------------------------------------------------------#
# 3. Compute one-sample Wilcoxon test
#--------------------------------------------------------#

?wilcox.test

res <- wilcox.test(df$weight, mu = 25)
res <- wilcox.test(df$weight, mu = 25, alternative = 'two.sided')
res <- wilcox.test(df$weight, mu = 25, alternative = 'less')
res <- wilcox.test(df$weight, mu = 25, alternative = 'greater')
class(res)
mode(res)
names(res)
