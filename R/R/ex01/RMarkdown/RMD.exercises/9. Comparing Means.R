...
#--------------------------------------------------------#
# 4. Preleminary test to check paired t-test assumptions
#--------------------------------------------------------#

d <- with(df, weight[group == 'before'] - weight[group == 'after'])
d
...
#--------------------------------------------------------#
# 5. Compute paired samples t-test
#--------------------------------------------------------#

fit <- t.test(before, after, paired = T)
fit <- t.test(before, after, paired = T, alternative = 'two.sided')
fit <- t.test(before, after, paired = T, alternative = 'left')
fit <- t.test(before, after, paired = T, alternative = 'greater')
fit

fit <- t.test(weight ~ group, data = df, paired = T)
fit <- t.test(weight ~ group, data = df, paired = T, alternative = 'two.sided')
fit <- t.test(weight ~ group, data = df, paired = T, alternative = 'left')
fit <- t.test(weight ~ group, data = df, paired = T, alternative = 'greater')
fit
