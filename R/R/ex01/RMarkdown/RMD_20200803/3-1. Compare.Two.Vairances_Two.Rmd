...
### 1. F-Test
  ...
  - When to use ?  
      ...
      + When you want to perform a **two samples t-test** to check the equality 
		    of the variances of the two samples
      + When you want to compare the variability of a new measurement method to 
...
### 2. Research questions and statistical hypotheses
  ...
  - Typical research questions are:
    + whether the variance of group A (σ^2^~A~) is **equal** to the variance of group B (σ^2^~B~)?
    + whether the variance of group A (σ^2^~A~) is **less** than the variance of group B (σ^2^~B~)?
    + whether the variance of group A (σ^2^~A~) is **greather** than the variance of group B (σ^2^~B~)?
    
  - In statistics, we can define the corresponding **null hypothesis (H~0~)** as follow:
    + H0 : σ^2^~A~ = σ^2^~B~ (two-tailed test)
    + H0 : σ^2^~A~ > σ^2^~B~ (one-tailed test)
    + H0 : σ^2^~A~ < σ^2^~B~ (one-tailed test)
  
  - the corresponding **alternative hypotheses (H~1~)** are as follow:
    + H1 : σ^2^~A~ ≠ σ^2^~B~ (different)
    + H1 : σ^2^~A~ ≤ σ^2^~B~  (greater)
    + H1 : σ^2^~A~ ≥ σ^2^~B~   (less)
...
## 4. Compute F-test
  ...
  - R function  
		`var.test{stats}` can be used to compare **two variances** as follow:
...
we start by displaying a random sample of 10 rows using the function **`sample_n{dplyr}`** :

```{r code2, message=FALSE, warning=FALSE, include=TRUE, echo=TRUE}
library("dplyr")

sample_n(my_data, 10, F)
```
...
### 6. Preliminary test to check F-test assumptions
  ...
  - `Shapiro-Wilk` test can be used to test whether the normal assumption holds. 
		...
		evaluate the normality of a variable. `Q-Q plot` draws the correlation between a given sample and the normal distribution
